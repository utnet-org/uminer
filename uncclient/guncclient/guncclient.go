...
// Package gethclient provides an RPC client for gunc-specific APIs.
package gethclient
...
// Client is a wrapper around rpc.Client that implements gunc-specific functionality.
//
// If you want to use the standardized Utility RPC functionality, use ethclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
func (ec *Client) CreateAccessList(ctx context.Context, msg utility.CallMsg) (*types.AccessList, uint64, string, error) {
	...
	if err := ec.c.CallContext(ctx, &result, "unc_createAccessList", toCallArg(msg)); err != nil {
		return nil, 0, "", err
	}
...
func (ec *Client) GetProof(ctx context.Context, account common.Address, keys []string, blockNumber *big.Int) (*AccountResult, error) {
	...
	err := ec.c.CallContext(ctx, &res, "unc_getProof", account, keys, toBlockNumArg(blockNumber))
...
func (ec *Client) CallContract(ctx context.Context, msg utility.CallMsg, blockNumber *big.Int, overrides *map[common.Address]OverrideAccount) ([]byte, error) {
	var hex hexutil.Bytes
	err := ec.c.CallContext(
		ctx, &hex, "unc_call", toCallArg(msg),
...
// the message call.
//
// blockOverrides specifies block fields exposed to the UVM that can be overridden for the call.
//
// Please use ethclient.CallContract instead if you don't need the override functionality.
func (ec *Client) CallContractWithBlockOverrides(ctx context.Context, msg utility.CallMsg, blockNumber *big.Int, overrides *map[common.Address]OverrideAccount, blockOverrides BlockOverrides) ([]byte, error) {
	var hex hexutil.Bytes
	err := ec.c.CallContext(
		ctx, &hex, "unc_call", toCallArg(msg),
...
// GCStats retrieves the current garbage collection stats from a gunc node.
...
// MemStats retrieves the current memory stats from a gunc node.
...
// GetNodeInfo retrieves the node info of a gunc node.
