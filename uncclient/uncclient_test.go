...
// Verify that Client implements the utility interfaces.
var (
	_ = utility.ChainReader(&Client{})
	_ = utility.TransactionReader(&Client{})
	_ = utility.ChainStateReader(&Client{})
	_ = utility.ChainSyncReader(&Client{})
	_ = utility.ContractCaller(&Client{})
	_ = utility.GasEstimator(&Client{})
	_ = utility.GasPricer(&Client{})
	_ = utility.LogFilterer(&Client{})
	_ = utility.PendingStateReader(&Client{})
	// _ = utility.PendingStateEventer(&Client{})
	_ = utility.PendingContractCaller(&Client{})
)
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == utility.NotFound {
		t.Fatal("error should not be nil/notfound")
	}

	// Test tx in block not found.
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != utility.NotFound {
		t.Fatal("error should be utility.NotFound")
	}
}
