...
import (
	...
	"github.com/yanhuangpai/go-utility/internal/jsre"
	"github.com/yanhuangpai/go-utility/internal/jsre/deps"
	"github.com/yanhuangpai/go-utility/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client         // RPC client to execute Utility requests through
...
var defaultAPIs = map[string]string{"unc": "1.0", "net": "1.0", "debug": "1.0"}
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines unc.getBalance(unc.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gunc instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	message := "Welcome to the Gunc JavaScript console!\n\n"

	// Print some generic Gunc metadata
	if res, err := c.jsre.Run(`
		var message = "instance: " + web3.version.node + "\n";
		try {
			message += "coinbase: " + unc.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + unc.blockNumber + " (" + new Date(1000 * unc.getBlock(unc.blockNumber).timestamp) + ")\n";
		try {
