...
import (
	...
	"github.com/yanhuangpai/go-utility/crypto/signify"
	"github.com/yanhuangpai/go-utility/internal/build"
	"github.com/yanhuangpai/go-utility/params"
)

var (
	// Files that end up in the gunc*.zip archive.
	gethArchiveFiles = []string{
		"COPYING",
		executablePath("gunc"),
	}

	// Files that end up in the gunc-alltools*.zip archive.
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert Utility contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
			Description: "Utility bootnode.",
		},
		{
			BinaryName:  "uvm",
			Description: "Developer utility version of the UVM (Utility Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
		},
		{
			BinaryName:  "gunc",
			Description: "Utility CLI client.",
		},
		{
			BinaryName:  "rlpdump",
		...
		{
			BinaryName:  "clef",
			Description: "Utility account management tool.",
		},
	}
...
func buildFlags(env build.Environment, staticLinking bool, buildTags []string) (flags []string) {
	var ld []string
	if env.Commit != "" {
		ld = append(ld, "-X", "github.com/yanhuangpai/go-utility/internal/version.gitCommit="+env.Commit)
		ld = append(ld, "-X", "github.com/yanhuangpai/go-utility/internal/version.gitDate="+env.Date)
	}
...
func doArchive(cmdline []string) {
	...
	var (
		...
		gunc     = "gunc-" + basegeth + ext
		alltools = "gunc-alltools-" + basegeth + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(gunc, gethArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{gunc, alltools} {
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with gunc-release.pub"
...
func doDocker(cmdline []string) {
	var (
		image    = flag.Bool("image", false, `If to build and push an arch specific docker image`)
		manifest = flag.String("manifest", "", `Push a multi-arch docker image for the specified architectures (usually "amd64,arm64")`)
		upload   = flag.String("upload", "", `Where to upload the docker image (usually "utility/client-go")`)
	)
	flag.CommandLine.Parse(cmdline)
	...
	//  - utility/client-go:latest                            - Pushes to the master branch, Gunc only
	//  - utility/client-go:stable                            - Version tag publish on GitHub, Gunc only
	//  - utility/client-go:alltools-latest                   - Pushes to the master branch, Gunc & tools
	//  - utility/client-go:alltools-stable                   - Version tag publish on GitHub, Gunc & tools
	//  - utility/client-go:release-<major>.<minor>           - Version tag publish on GitHub, Gunc only
	//  - utility/client-go:alltools-release-<major>.<minor>  - Version tag publish on GitHub, Gunc & tools
	//  - utility/client-go:v<major>.<minor>.<patch>          - Version tag publish on GitHub, Gunc only
	//  - utility/client-go:alltools-v<major>.<minor>.<patch> - Version tag publish on GitHub, Gunc & tools
	var tags []string
	...
	if len(*manifest) != 0 {
		...
		// Assemble and push the Gunc manifest image
		for _, tag := range tags {
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the source package (usually "utility/utility")`)
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "gunc-ci")`)
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, pkg := range debPackages {
		for distro, goboot := range debDistroGoBoots {
			// Prepare the debian package with the go-utility sources.
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = os.MkdirTemp("", "gunc-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
type debPackage struct {
	Name        string          // the name of the Debian package to produce, e.g. "utility"
...
type debMetadata struct {
	...
	// go-utility version being built. Note that this
...
func newDebMetadata(distro, goboot, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "Utility Builds <fjl@utility.org>"
	}
	return debMetadata{
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	// first section contains the gunc binary, second section holds the dev tools.
	...
	installer, err := filepath.Abs("gunc-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
	if err != nil {
	...
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "gunc.nsi"),
	)
