...
// AdminAPI is the collection of Utility full node related APIs for node
// administration.
...
func NewAdminAPI(unc *Utility) *AdminAPI {
	return &AdminAPI{unc: unc}
}
...
func (api *AdminAPI) ExportChain(file string, first *uint64, last *uint64) (bool, error) {
	...
	if first != nil && last == nil {
		head := api.unc.BlockChain().CurrentHeader().Number.Uint64()
		last = &head
	}
	...
	if first != nil {
		if err := api.unc.BlockChain().ExportN(writer, *first, *last); err != nil {
			return false, err
		}
	} else if err := api.unc.BlockChain().Export(writer); err != nil {
		return false, err
	}
	return true, nil
}
...
func (api *AdminAPI) ImportChain(file string) (bool, error) {
	...
	for batch := 0; ; batch++ {
		...
		if hasAllBlocks(api.unc.BlockChain(), blocks) {
			blocks = blocks[:0]
			continue
		}
		// Import the batch and reset the buffer
		if _, err := api.unc.BlockChain().InsertChain(blocks); err != nil {
