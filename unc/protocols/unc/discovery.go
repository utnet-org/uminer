...
// enrEntry is the ENR entry which advertises `unc` protocol on the discovery.
...
// StartENRUpdater starts the `unc` ENR updater loop, which listens for chain
...
// currentENREntry constructs an `unc` ENR entry based on the current state of the chain.
