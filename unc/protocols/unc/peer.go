...
// Peer is a collection of relevant information we have about a `unc` peer.
type Peer struct {
...
// Version retrieves the peer's negotiated `unc` protocol version.
...
// KnownBlock returns if peer is known to already have a block.
func (p *Peer) KnownBlock(hash common.Hash) bool {
	return p.knownBlocks.Contains(hash)
}

// KnownTransaction returns if peer is known to already have a transaction.
...
// Contains returns if the given item is in the set.
