...
type Backend interface {
	...
	// RunPeer is invoked when a peer joins on the `unc` protocol. The handler
...
// MakeProtocols constructs the P2P protocol definitions for `unc`.
func MakeProtocols(backend Backend, network uint64, dnsdisc enode.Iterator) []p2p.Protocol {
	protocols := make([]p2p.Protocol, 0, len(ProtocolVersions))
	for _, version := range ProtocolVersions {
		// Blob transactions require unc/68 announcements, disable everything else
...
// NodeInfo represents a short summary of the `unc` sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Utility network ID (1=Mainnet, Goerli=5)
...
// nodeInfo retrieves some `unc` protocol metadata about the running host node.
...
// Handle is invoked whenever an `unc` connection is made that successfully passes
// the protocol handshake. This method will keep processing messages until the
// connection is torn down.
func Handle(backend Backend, peer *Peer) error {
	for {
		if err := handleMessage(backend, peer); err != nil {
			peer.Log().Debug("Message handling failed in `unc`", "err", err)
			return err
		}
	}
}
