...
var (
	...
	// snap protocol without advertising the unc main protocol.
	errSnapWithoutEth = errors.New("peer connected on snap without compatible unc support")
)

// peerSet represents the collection of active peers currently participating in
// the `unc` protocol, with or without the `snap` extension.
type peerSet struct {
	peers     map[string]*ethPeer // Peers connected on the `unc` protocol
	snapPeers int                 // Number of `snap` compatible peers for connection prioritization

	snapWait map[string]chan *snap.Peer // Peers connected on `unc` waiting for their snap extension
	snapPend map[string]*snap.Peer      // Peers connected on the `snap` protocol, but not yet on `unc`
...
// registerSnapExtension unblocks an already connected `unc` peer waiting for its
// `snap` extension, or if no such peer exists, tracks the extension for the time
// being until the `unc` main protocol starts looking for it.
func (ps *peerSet) registerSnapExtension(peer *snap.Peer) error {
	// Reject the peer if it advertises `snap` without `unc` as `snap` is only a
	// satellite protocol meaningful with the chain selection of `unc`
	if !peer.RunningCap(unc.ProtocolName, unc.ProtocolVersions) {
	...
	// Inject the peer into an `unc` counterpart is available, otherwise save for later
...
// by the peerset.
func (ps *peerSet) waitSnapExtension(peer *unc.Peer) (*snap.Peer, error) {
...
// registerPeer injects a new `unc` peer into the working set, or returns an error
// if the peer is already known.
func (ps *peerSet) registerPeer(peer *unc.Peer, ext *snap.Peer) error {
...
// len returns if the current number of `unc` peers in the set. Since the `snap`
// peers are tied to the existence of an `unc` connection, that will always be a
// subset of `unc`.
...
func (ps *peerSet) peerWithHighestTD() *unc.Peer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()
