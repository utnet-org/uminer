...
// information related to the Utility protocol such as blocks, transactions and logs.
...
// `unc_getFilterChanges` polling method that is also used for log filters.
			...
			case txs := <-txs:
				...
				for _, tx := range txs {
					if fullTx != nil && *fullTx {
						rpcTx := uncapi.NewRPCPendingTransaction(tx, latest, chainConfig)
...
// It is part of the filter package since polling goes with unc_getFilterChanges.
...
func (api *FilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(utility.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
// Same as utility.FilterQuery but with UnmarshalJSON() method.
type FilterCriteria utility.FilterQuery
...
func (api *FilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	...
	logsSub, err := api.events.SubscribeLogs(utility.FilterQuery(crit), logs)
	if err != nil {
		return "", err
	}
...
func (api *FilterAPI) GetFilterChanges(id rpc.ID) (interface{}, error) {
	...
	if f, found := api.filters[id]; found {
		...
		case PendingTransactionsSubscription:
			if f.fullTx {
				txs := make([]*uncapi.RPCTransaction, 0, len(f.txs))
				for _, tx := range f.txs {
					txs = append(txs, uncapi.NewRPCPendingTransaction(tx, latest, chainConfig))
				}
				f.txs = nil
				return txs, nil
			} else {
