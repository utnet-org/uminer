...
// DebugAPI is the collection of Utility full node APIs for debugging the
// protocol.
...
func NewDebugAPI(unc *Utility) *DebugAPI {
	return &DebugAPI{unc: unc}
}
...
func (api *DebugAPI) DumpBlock(blockNr rpc.BlockNumber) (state.Dump, error) {
	...
	if blockNr == rpc.PendingBlockNumber {
		...
		_, stateDb := api.unc.miner.Pending()
		if stateDb == nil {
	...
	case rpc.LatestBlockNumber:
		header = api.unc.blockchain.CurrentBlock()
	case rpc.FinalizedBlockNumber:
		header = api.unc.blockchain.CurrentFinalBlock()
	case rpc.SafeBlockNumber:
		header = api.unc.blockchain.CurrentSafeBlock()
	default:
		block := api.unc.blockchain.GetBlockByNumber(uint64(blockNr))
		if block == nil {
	...
	stateDb, err := api.unc.BlockChain().StateAt(header.Root)
	if err != nil {
...
func (api *DebugAPI) Preimage(ctx context.Context, hash common.Hash) (hexutil.Bytes, error) {
	if preimage := rawdb.ReadPreimage(api.unc.ChainDb(), hash); preimage != nil {
		return preimage, nil
	}
...
func (api *DebugAPI) GetBadBlocks(ctx context.Context) ([]*BadBlockArgs, error) {
	var (
		blocks  = rawdb.ReadAllBadBlocks(api.unc.chainDb)
	...
	if number, ok := blockNrOrHash.Number(); ok {
		if number == rpc.PendingBlockNumber {
			...
			case rpc.SafeBlockNumber:
				header = api.unc.blockchain.CurrentSafeBlock()
			default:
				block := api.unc.blockchain.GetBlockByNumber(uint64(number))
				if block == nil {
		...
		stateDb, err = api.unc.BlockChain().StateAt(block.Root())
		if err != nil {
			return state.IteratorDump{}, err
		}
	} else {
...
func (api *DebugAPI) StorageRangeAt(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash, txIndex int, contractAddress common.Address, keyStart hexutil.Bytes, maxResult int) (StorageRangeResult, error) {
	...
	block, err := api.unc.APIBackend.BlockByNumberOrHash(ctx, blockNrOrHash)
	if err != nil {
	...
	_, _, statedb, release, err := api.unc.stateAtTransaction(ctx, block, txIndex, 0)
	if err != nil {
		return StorageRangeResult{}, err
	}
	defer release()
...
func (api *DebugAPI) GetModifiedAccountsByNumber(startNum uint64, endNum *uint64) ([]common.Address, error) {
	...
	startBlock = api.unc.blockchain.GetBlockByNumber(startNum)
	if startBlock == nil {
	...
	if endNum == nil {
		endBlock = startBlock
		startBlock = api.unc.blockchain.GetBlockByHash(startBlock.ParentHash())
		if startBlock == nil {
			return nil, fmt.Errorf("block %x has no parent", endBlock.Number())
		}
	} else {
		endBlock = api.unc.blockchain.GetBlockByNumber(*endNum)
		if endBlock == nil {
...
func (api *DebugAPI) GetModifiedAccountsByHash(startHash common.Hash, endHash *common.Hash) ([]common.Address, error) {
	var startBlock, endBlock *types.Block
	startBlock = api.unc.blockchain.GetBlockByHash(startHash)
	if startBlock == nil {
	...
	if endHash == nil {
		...
		endBlock = api.unc.blockchain.GetBlockByHash(*endHash)
		if endBlock == nil {
...
func (api *DebugAPI) getModifiedAccounts(startBlock, endBlock *types.Block) ([]common.Address, error) {
	...
	triedb := api.unc.BlockChain().TrieDB()
...
func (api *DebugAPI) GetAccessibleState(from, to rpc.BlockNumber) (uint64, error) {
	if api.unc.blockchain.TrieDB().Scheme() == rawdb.PathScheme {
	...
	var resolveNum = func(num rpc.BlockNumber) (uint64, error) {
		...
		if num.Int64() < 0 {
			block := api.unc.blockchain.CurrentBlock()
			if block == nil {
	...
	for i := int64(start); i != int64(end); i += delta {
		...
		h := api.unc.BlockChain().GetHeaderByNumber(uint64(i))
		if h == nil {
			return 0, fmt.Errorf("missing header %d", i)
		}
		if ok, _ := api.unc.ChainDb().Has(h.Root[:]); ok {
			return uint64(i), nil
		}
	}
...
func (api *DebugAPI) SetTrieFlushInterval(interval string) error {
	...
	api.unc.blockchain.SetTrieFlushInterval(t)
	return nil
}
...
func (api *DebugAPI) GetTrieFlushInterval() (string, error) {
	...
	return api.unc.blockchain.GetTrieFlushInterval().String(), nil
}
