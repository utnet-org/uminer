...
// ethPeerInfo represents a short summary of the `unc` sub-protocol metadata known
// about a connected peer.
type ethPeerInfo struct {
	Version uint `json:"version"` // Utility protocol version negotiated
}

// ethPeer is a wrapper around unc.Peer to maintain a few extra metadata.
type ethPeer struct {
	*unc.Peer
	snapExt *snapPeer // Satellite `snap` connection
}

// info gathers and returns some `unc` protocol metadata known about a peer.
