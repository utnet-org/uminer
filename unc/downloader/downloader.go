...
var (
	...
	maxQueuedHeaders            = 32 * 1024                         // [unc/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	stateDB uncdb.Database // Database to state sync into (and deduplicate via)
	...
	syncStartBlock uint64    // Head snap block when Gunc was started
...
func New(stateDb uncdb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn, success func()) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
// these are zero.
func (d *Downloader) Progress() utility.SyncProgress {
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td, ttd *big.Int, beaconMode bool) (err error) {
	...
	if !beaconMode {
		log.Debug("Synchronising with the network", "peer", p.id, "unc", p.version, "head", hash, "td", td, "mode", mode)
	} else {
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	if index, err := d.blockchain.InsertChain(blocks); err != nil {
		if index < len(results) {
			...
			// The importer will put jointly a new list of blocks to import, which is a superset
