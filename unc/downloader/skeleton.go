...
// the process. This is usually the case when Gunc is shutting down and some events
...
// data on disk when two subchains are joined jointly.
...
// The update might request if to forcefully change the target, or only try to
...
type headUpdate struct {
	...
	force  bool          // If to force the update or only extend if possible
...
//
// Opposed to the original Utility block synchronization which is trustless (and
...
// not from within (vs. legacy Utility sync).
//
...
func (s *skeleton) startup() {
	...
	for {
		...
		case errc := <-s.terminate:
			// No head was announced but Gunc is shutting down
			errc <- nil
			return
		...
		case event := <-s.headEvents:
			...
			for {
				...
				default:
					...
					// error. Abort and wait until Gunc requests a termination.
					errc := <-s.terminate
					errc <- err
					return
				}
	...
	if status := rawdb.ReadSkeletonSyncStatus(s.db); len(status) > 0 {
		...
		if err := json.Unmarshal(status, s.progress); err != nil {
			...
			for len(s.progress.Subchains) > 0 {
				// If the last chain is above the new head, delete completely
...
func (s *skeleton) assignTasks(success chan *headerResponse, fail chan *headerRequest, cancel chan struct{}) {
	...
	for _, peer := range s.idles {
		...
		idlers.caps = append(idlers.caps, s.peers.rates.Capacity(peer.id, unc.BlockHeadersMsg, targetTTL))
	}
...
func (s *skeleton) executeTask(peer *peerConnection, req *headerRequest) {
	...
	case <-timeoutTimer.C:
		...
		s.peers.rates.Update(peer.id, unc.BlockHeadersMsg, 0, 0)
		s.scheduleRevertRequest(req)
	...
	case res := <-resCh:
		...
		headers := *res.Res.(*unc.BlockHeadersRequest)

		headerReqTimer.Update(time.Since(start))
		s.peers.rates.Update(peer.id, unc.BlockHeadersMsg, res.Time, len(headers))
...
func (s *skeleton) processResponse(res *headerResponse) (linked bool, merged bool) {
	...
	// If the response is valid, we can mark the peer as idle and notify
