...
type skeletonTestPeer struct {
	...
	dropped atomic.Uint64 // Flag if the peer was dropped (stop responding)
}
...
func (p *skeletonTestPeer) RequestHeadersByNumber(origin uint64, amount int, skip int, reverse bool, sink chan *unc.Response) (*unc.Request, error) {
	...
	if headers == nil {
		...
		if len(p.headers) > int(origin) { // Don't serve headers if we're missing the origin
			for i := 0; i < amount; i++ {
				...
				// attack by sending/receiving those over unc.
...
func (p *skeletonTestPeer) RequestHeadersByHash(common.Hash, int, int, bool, chan *unc.Response) (*unc.Request, error) {
	panic("skeleton sync must not request headers by hash")
}

func (p *skeletonTestPeer) RequestBodies([]common.Hash, chan *unc.Response) (*unc.Request, error) {
	panic("skeleton sync must not request block bodies")
}

func (p *skeletonTestPeer) RequestReceipts([]common.Hash, chan *unc.Response) (*unc.Request, error) {
...
func TestSkeletonSyncExtend(t *testing.T) {
	...
	tests := []struct {
		...
		newstate []*subchain   // Expected sync state after the reorg
		err      error         // If extension succeeds or not
	}{
...
func TestSkeletonSyncRetrievals(t *testing.T) {
	...
	tests := []struct {
		fill          bool // If to run a real backfiller in this test case
		unpredictable bool // If to ignore drops/serves due to uncertain packet assignments
	...
	for i, tt := range tests {
		...
		for _, peer := range tt.peers {
			peerset.Register(newPeerConnection(peer.id, unc.ETH67, peer, log.New("id", peer.id)))
		}
		...
		if tt.newPeer != nil {
			if err := peerset.Register(newPeerConnection(tt.newPeer.id, unc.ETH67, tt.newPeer, log.New("id", tt.newPeer.id))); err != nil {
