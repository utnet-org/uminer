...
type peerConnection struct {
	...
	version uint       // Unc protocol version number to switch strategies
...
type Peer interface {
	...
	RequestHeadersByHash(common.Hash, int, int, bool, chan *unc.Response) (*unc.Request, error)
	RequestHeadersByNumber(uint64, int, int, bool, chan *unc.Response) (*unc.Request, error)

	RequestBodies([]common.Hash, chan *unc.Response) (*unc.Request, error)
	RequestReceipts([]common.Hash, chan *unc.Response) (*unc.Request, error)
}
...
func (p *peerConnection) UpdateHeaderRate(delivered int, elapsed time.Duration) {
	p.rates.Update(unc.BlockHeadersMsg, elapsed, delivered)
}
...
func (p *peerConnection) UpdateBodyRate(delivered int, elapsed time.Duration) {
	p.rates.Update(unc.BlockBodiesMsg, elapsed, delivered)
}
...
func (p *peerConnection) UpdateReceiptRate(delivered int, elapsed time.Duration) {
	p.rates.Update(unc.ReceiptsMsg, elapsed, delivered)
}

// HeaderCapacity retrieves the peer's header download allowance based on its
// previously discovered throughput.
func (p *peerConnection) HeaderCapacity(targetRTT time.Duration) int {
	cap := p.rates.Capacity(unc.BlockHeadersMsg, targetRTT)
...
func (p *peerConnection) BodyCapacity(targetRTT time.Duration) int {
	cap := p.rates.Capacity(unc.BlockBodiesMsg, targetRTT)
...
func (p *peerConnection) ReceiptCapacity(targetRTT time.Duration) int {
	cap := p.rates.Capacity(unc.ReceiptsMsg, targetRTT)
...
func newPeerSet() *peerSet {
	return &peerSet{
		...
		rates: msgrate.NewTrackers(log.New("proto", "unc")),
	}
}
