...
type typedQueue interface {
	...
	request(peer *peerConnection, req *fetchRequest, resCh chan *unc.Response) (*unc.Request, error)
	...
	deliver(peer *peerConnection, packet *unc.Response) (int, error)
}
...
func (d *Downloader) concurrentFetch(queue typedQueue, beaconMode bool) error {
	...
	responses := make(chan *unc.Response)

	// Track the currently active requests and their timeout order
	pending := make(map[string]*unc.Request)
	defer func() {
	...
	ordering := make(map[*unc.Request]int)
	timeouts := prque.New[int64, *unc.Request](func(data *unc.Request, index int) {
		ordering[data] = index
	})
	...
	// the pending request completely as that would prevent a late response from
	// being delivered, thus never unblocking the peer.
	stales := make(map[string]*unc.Request)
	defer func() {
		// Abort all requests on sync cycle cancellation. The requests may still
		// be fulfilled by the remote side, but the dispatcher will not wait to
		// deliver them since nobody's going to be listening.
