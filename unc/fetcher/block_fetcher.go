...
import (
	...
	"github.com/yanhuangpai/go-utility/common/prque"
...
var (
	blockAnnounceInMeter   = metrics.NewRegisteredMeter("unc/fetcher/block/announces/in", nil)
	blockAnnounceOutTimer  = metrics.NewRegisteredTimer("unc/fetcher/block/announces/out", nil)
	blockAnnounceDropMeter = metrics.NewRegisteredMeter("unc/fetcher/block/announces/drop", nil)
	blockAnnounceDOSMeter  = metrics.NewRegisteredMeter("unc/fetcher/block/announces/dos", nil)

	blockBroadcastInMeter   = metrics.NewRegisteredMeter("unc/fetcher/block/broadcasts/in", nil)
	blockBroadcastOutTimer  = metrics.NewRegisteredTimer("unc/fetcher/block/broadcasts/out", nil)
	blockBroadcastDropMeter = metrics.NewRegisteredMeter("unc/fetcher/block/broadcasts/drop", nil)
	blockBroadcastDOSMeter  = metrics.NewRegisteredMeter("unc/fetcher/block/broadcasts/dos", nil)

	headerFetchMeter = metrics.NewRegisteredMeter("unc/fetcher/block/headers", nil)
	bodyFetchMeter   = metrics.NewRegisteredMeter("unc/fetcher/block/bodies", nil)

	headerFilterInMeter  = metrics.NewRegisteredMeter("unc/fetcher/block/filter/headers/in", nil)
	headerFilterOutMeter = metrics.NewRegisteredMeter("unc/fetcher/block/filter/headers/out", nil)
	bodyFilterInMeter    = metrics.NewRegisteredMeter("unc/fetcher/block/filter/bodies/in", nil)
	bodyFilterOutMeter   = metrics.NewRegisteredMeter("unc/fetcher/block/filter/bodies/out", nil)
)
...
type headerRequesterFn func(common.Hash, chan *unc.Response) (*unc.Request, error)

// bodyRequesterFn is a callback type for sending a body retrieval request.
type bodyRequesterFn func([]common.Hash, chan *unc.Response) (*unc.Request, error)
...
type BlockFetcher struct {
	...
	fetchingHook       func([]common.Hash)               // Method to call upon starting a block (unc/61) or header (unc/62) fetch
	completingHook     func([]common.Hash)               // Method to call upon starting a block body fetch (unc/62)
	importedHook       func(*types.Header, *types.Block) // Method to call upon successful header or block import (both unc/61 and unc/62)
}
				...
				go func(peer string) {
					...
					for _, hash := range hashes {
						...
						go func(hash common.Hash) {
							...
							case res := <-resCh:
								res.Done <- nil
								f.FilterHeaders(peer, *res.Res.(*unc.BlockHeadersRequest), time.Now().Add(res.Time))
			...
			for peer, hashes := range request {
				...
				go func(peer string, hashes []common.Hash) {
					...
					case res := <-resCh:
						...
						txs, uncles, _ := res.Res.(*unc.BlockBodiesResponse).Unpack()
