...
func (f *fetcherTester) makeHeaderFetcher(peer string, blocks map[common.Hash]*types.Block, drift time.Duration) headerRequesterFn {
	...
	return func(hash common.Hash, sink chan *unc.Response) (*unc.Request, error) {
...
func (f *fetcherTester) makeBodyFetcher(peer string, blocks map[common.Hash]*types.Block, drift time.Duration) bodyRequesterFn {
	...
	return func(hashes []common.Hash, sink chan *unc.Response) (*unc.Request, error) {
		...
		// Return on a new thread
		bodies := make([]*unc.BlockBody, len(transactions))
		for i, txs := range transactions {
			bodies[i] = &unc.BlockBody{
				Transactions: txs,
				Uncles:       uncles[i],
			}
		}
...
func testConcurrentAnnouncements(t *testing.T, light bool) {
	...
	firstHeaderWrapper := func(hash common.Hash, sink chan *unc.Response) (*unc.Request, error) {
		counter.Add(1)
		return firstHeaderFetcher(hash, sink)
	}
	secondHeaderWrapper := func(hash common.Hash, sink chan *unc.Response) (*unc.Request, error) {
		counter.Add(1)
...
func testPendingDeduplication(t *testing.T, light bool) {
	...
	var counter atomic.Uint32
	headerWrapper := func(hash common.Hash, sink chan *unc.Response) (*unc.Request, error) {
		counter.Add(1)

		// Simulate a long running fetch
		resink := make(chan *unc.Response)
