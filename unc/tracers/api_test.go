...
func TestTraceCall(t *testing.T) {
	...
	var testSuite = []struct {
		...
		},
		{
			...
			config: &TraceCallConfig{
				BlockOverrides: &uncapi.BlockOverrides{Number: (*hexutil.Big)(big.NewInt(0x1337))},
			},
			expectErr: nil,
	...
	var testSuite = []struct {
		...
		// Call which can only succeed if state is state overridden
		{
			...
			config: &TraceCallConfig{
				StateOverrides: &uncapi.StateOverride{
					randomAccounts[0].addr: uncapi.OverrideAccount{Balance: newRPCBalance(new(big.Int).Mul(big.NewInt(1), big.NewInt(params.Unicrpyt)))},
				},
			},
		...
		//  }
		{
			...
			config: &TraceCallConfig{
				//Tracer: &tracer,
				StateOverrides: &uncapi.StateOverride{
					randomAccounts[2].addr: uncapi.OverrideAccount{
		...
		*/
		...
		{ // No state override
			...
			config: &TraceCallConfig{
				StateOverrides: &uncapi.StateOverride{
					storageAccount: uncapi.OverrideAccount{
