...
func testCallTracer(tracerName string, dirPath string, t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(file.Name(), ".json")), func(t *testing.T) {
			...
			uvm := vm.NewEVM(context, txContext, statedb, test.Genesis.Config, vm.Config{Tracer: tracer})
			msg, err := core.TransactionToMessage(tx, signer, nil)
			if err != nil {
				t.Fatalf("failed to prepare transaction for tracing: %v", err)
			}
			vmRet, err := core.ApplyMessage(uvm, msg, new(core.GasPool).AddGas(tx.Gas()))
			if err != nil {
...
func benchTracer(tracerName string, test *callTracerTest, b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		st := core.NewStateTransition(uvm, msg, new(core.GasPool).AddGas(tx.Gas()))
...
func TestInternals(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			uvm := vm.NewEVM(context, txContext, statedb, params.MainnetChainConfig, vm.Config{Tracer: tc.tracer})
			msg := &core.Message{
				To:                &to,
				From:              origin,
				Value:             big.NewInt(0),
			...
			st := core.NewStateTransition(uvm, msg, new(core.GasPool).AddGas(msg.GasLimit))
			if _, err := st.TransitionDb(); err != nil {
