...
// Config are the configuration options for structured logger the UVM
type Config struct {
...
// StructLog is emitted to the UVM each cycle and lists information about the current internal state
...
// StructLogger is an UVM state logger and implements EVMLogger.
//
...
func (l *StructLogger) CaptureStart(env *vm.UVM, from common.Address, to common.Address, create bool, input []byte, gas uint64, value *big.Int) {
	l.env = env
}
...
func (l *StructLogger) CaptureState(pc uint64, op vm.OpCode, gas, cost uint64, scope *vm.ScopeContext, rData []byte, depth int, err error) {
	...
	// create a new snapshot of the UVM.
...
func (t *mdLogger) CaptureStart(env *vm.UVM, from common.Address, to common.Address, create bool, input []byte, gas uint64, value *big.Int) {
	t.env = env
	if !create {
...
// ExecutionResult groups all structured logs emitted by the UVM
...
// StructLogRes stores a structured log emitted by the UVM while replaying a
...
// formatLogs formats UVM returned structured logs for json output
