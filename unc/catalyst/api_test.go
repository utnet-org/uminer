...
import (
	...
	beaconConsensus "github.com/yanhuangpai/go-utility/consensus/beacon"
...
var (
	...
	// testAddr is the Utility address of the tester account.
...
func TestEth2AssembleBlockWithAnotherBlocksTxs(t *testing.T) {
	...
	api.unc.TxPool().Add(txs, false, true)
	blockParams := engine.PayloadAttributes{
...
func startEthService(t *testing.T, genesis *core.Genesis, blocks []*types.Block) (*node.Node, *unc.Utility) {
	t.Helper()
	...
	ethservice.SetUnicrpytbase(testAddr)
...
func setupBlocks(t *testing.T, ethservice *unc.Utility, n int, parent *types.Header, callback func(parent *types.Header), withdrawals [][]*types.Withdrawal) []*types.Header {
	api := NewConsensusAPI(ethservice)
...
func setupBodies(t *testing.T) (*node.Node, *unc.Utility, []*types.Block) {
	genesis, blocks := generateMergeChain(10, true)
...
func TestGetBlockBodiesByHash(t *testing.T) {
	node, unc, blocks := setupBodies(t)
	api := NewConsensusAPI(unc)
	defer node.Close()
	...
	tests := []struct {
		...
		// First pow block
		{
			results: []*types.Body{unc.BlockChain().GetBlockByNumber(0).Body()},
			hashes:  []common.Hash{unc.BlockChain().GetBlockByNumber(0).Hash()},
		},
		// Last pow block
		{
