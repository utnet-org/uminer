...
import (
	...
	"github.com/yanhuangpai/go-utility/beacon/engine"
...
func Register(stack *node.Node, backend *unc.Utility) error {
	log.Warn("Engine API enabled", "protocol", "unc")
...
type ConsensusAPI struct {
	...
	//     persist any bad block information to disk as a bug in Gunc could end
	//     up blocking a valid chain, even if a later Gunc update would accept
	//     it.
	//   - Bad blocks will get forgotten after a certain threshold of import
	//     attempts and will be retried. The rationale is that if the network
	//     really-really-really tries to feed us a block, we should give it a
	//     new chance, perhaps us being racey instead of the block being legit
	//     bad (this happened in Gunc at a point with import vs. pending race).
	...
	// Gunc can appear to be stuck or do strange things if the beacon client is
...
func NewConsensusAPI(unc *unc.Utility) *ConsensusAPI {
	api := newConsensusAPIWithoutHeartbeat(unc)
	go api.heartbeat()
	return api
}

// newConsensusAPIWithoutHeartbeat creates a new consensus api for the SimulatedBeacon Node.
func newConsensusAPIWithoutHeartbeat(unc *unc.Utility) *ConsensusAPI {
	if unc.BlockChain().Config().TerminalTotalDifficulty == nil {
	...
	unc.Downloader().SetBadBlockCallback(api.setInvalidAncestor)
	return api
}
...
func (api *ConsensusAPI) ForkchoiceUpdatedV1(update engine.ForkchoiceStateV1, payloadAttributes *engine.PayloadAttributes) (engine.ForkChoiceResponse, error) {
	if payloadAttributes != nil {
		...
		if api.unc.BlockChain().Config().IsShanghai(api.unc.BlockChain().Config().LondonBlock, payloadAttributes.Timestamp) {
...
func (api *ConsensusAPI) forkchoiceUpdated(update engine.ForkchoiceStateV1, payloadAttributes *engine.PayloadAttributes) (engine.ForkChoiceResponse, error) {
	...
	// Check if we have the block yet in our database or not. If not, we'll
	// need to either trigger a sync, or to reject this forkchoice update for a
	// reason.
	block := api.unc.BlockChain().GetBlockByHash(update.HeadBlockHash)
	if block == nil {
		...
		// the future to resolve from the `unc` network, but it's an unexpected case
		...
		if merger := api.unc.Merger(); !merger.TDDReached() {
			merger.ReachTTD()
			api.unc.Downloader().Cancel()
		}
		...
		if err := api.unc.Downloader().BeaconSync(api.unc.SyncMode(), header, finalized); err != nil {
	...
	if block.Difficulty().BitLen() > 0 || block.NumberU64() == 0 {
		var (
			td  = api.unc.BlockChain().GetTd(update.HeadBlockHash, block.NumberU64())
			ptd = api.unc.BlockChain().GetTd(block.ParentHash(), block.NumberU64()-1)
			ttd = api.unc.BlockChain().Config().TerminalTotalDifficulty
		)
	...
	if rawdb.ReadCanonicalHash(api.unc.ChainDb(), block.NumberU64()) != update.HeadBlockHash {
		// Block is not canonical, set head.
		if latestValid, err := api.unc.BlockChain().SetCanonical(block); err != nil {
			return engine.ForkChoiceResponse{PayloadStatus: engine.PayloadStatusV1{Status: engine.INVALID, LatestValidHash: &latestValid}}, err
		}
	} else if api.unc.BlockChain().CurrentBlock().Hash() == update.HeadBlockHash {
		...
		log.Info("Ignoring beacon update to old head", "number", block.NumberU64(), "hash", update.HeadBlockHash, "age", common.PrettyAge(time.Unix(int64(block.Time()), 0)), "have", api.unc.BlockChain().CurrentBlock().Number)
	...
	if update.FinalizedBlockHash != (common.Hash{}) {
		if merger := api.unc.Merger(); !merger.PoSFinalized() {
			merger.FinalizePoS()
		}
		// If the finalized block is not in our canonical tree, somethings wrong
		finalBlock := api.unc.BlockChain().GetBlockByHash(update.FinalizedBlockHash)
		if finalBlock == nil {
			log.Warn("Final block not available in database", "hash", update.FinalizedBlockHash)
			return engine.STATUS_INVALID, engine.InvalidForkChoiceState.With(errors.New("final block not available in database"))
		} else if rawdb.ReadCanonicalHash(api.unc.ChainDb(), finalBlock.NumberU64()) != update.FinalizedBlockHash {
			log.Warn("Final block not in canonical chain", "number", block.NumberU64(), "hash", update.HeadBlockHash)
			return engine.STATUS_INVALID, engine.InvalidForkChoiceState.With(errors.New("final block not in canonical chain"))
		}
		// Set the finalized block
		api.unc.BlockChain().SetFinalized(finalBlock.Header())
	}
	// Check if the safe block hash is in our canonical tree, if not somethings wrong
	if update.SafeBlockHash != (common.Hash{}) {
		safeBlock := api.unc.BlockChain().GetBlockByHash(update.SafeBlockHash)
		if safeBlock == nil {
			log.Warn("Safe block not available in database")
			return engine.STATUS_INVALID, engine.InvalidForkChoiceState.With(errors.New("safe block not available in database"))
		}
		if rawdb.ReadCanonicalHash(api.unc.ChainDb(), safeBlock.NumberU64()) != update.SafeBlockHash {
			log.Warn("Safe block not in canonical chain")
			return engine.STATUS_INVALID, engine.InvalidForkChoiceState.With(errors.New("safe block not in canonical chain"))
		}
		// Set the safe block
		api.unc.BlockChain().SetSafe(safeBlock.Header())
	}
	...
	if payloadAttributes != nil {
		...
		payload, err := api.unc.Miner().BuildPayload(args)
		if err != nil {
...
func (api *ConsensusAPI) ExchangeTransitionConfigurationV1(config engine.TransitionConfigurationV1) (*engine.TransitionConfigurationV1, error) {
	...
	if ttd == nil || ttd.Cmp(config.TerminalTotalDifficulty.ToInt()) != 0 {
		log.Warn("Invalid TTD configured", "gunc", ttd, "beacon", config.TerminalTotalDifficulty)
		return nil, fmt.Errorf("invalid ttd: execution %v consensus %v", ttd, config.TerminalTotalDifficulty)
	}
	if config.TerminalBlockHash != (common.Hash{}) {
		if hash := api.unc.BlockChain().GetCanonicalHash(uint64(config.TerminalBlockNumber)); hash == config.TerminalBlockHash {
...
func (api *ConsensusAPI) NewPayloadV2(params engine.ExecutableData) (engine.PayloadStatusV1, error) {
	if api.unc.BlockChain().Config().IsShanghai(new(big.Int).SetUint64(params.Number), params.Timestamp) {
		if params.Withdrawals == nil {
	...
	if api.unc.BlockChain().Config().IsCancun(new(big.Int).SetUint64(params.Number), params.Timestamp) {
...
func (api *ConsensusAPI) newPayload(params engine.ExecutableData, versionedHashes []common.Hash, beaconRoot *common.Hash) (engine.PayloadStatusV1, error) {
	...
	//      api.unc.BlockChain().InsertBlockWithoutSetHead, where it is blocked on
	...
	// check if we already have the block locally.
	...
	if block := api.unc.BlockChain().GetBlockByHash(params.BlockHash); block != nil {
	...
	parent := api.unc.BlockChain().GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
	...
	var (
		ptd  = api.unc.BlockChain().GetTd(parent.Hash(), parent.NumberU64())
		ttd  = api.unc.BlockChain().Config().TerminalTotalDifficulty
		gptd = api.unc.BlockChain().GetTd(parent.ParentHash(), parent.NumberU64()-1)
	)
	if ptd.Cmp(ttd) < 0 {
	...
	if api.unc.SyncMode() != downloader.FullSync {
		return api.delayPayloadImport(block)
	}
	if !api.unc.BlockChain().HasBlockAndState(block.ParentHash(), block.NumberU64()-1) {
		api.remoteBlocks.put(block.Hash(), block.Header())
		log.Warn("State not available, ignoring new payload")
		return engine.PayloadStatusV1{Status: engine.ACCEPTED}, nil
	}
	log.Trace("Inserting block without sethead", "hash", block.Hash(), "number", block.Number)
	if err := api.unc.BlockChain().InsertBlockWithoutSetHead(block); err != nil {
...
func (api *ConsensusAPI) delayPayloadImport(block *types.Block) (engine.PayloadStatusV1, error) {
	...
	if err := api.unc.Downloader().BeaconExtend(api.unc.SyncMode(), block.Header()); err == nil {
...
// checkInvalidAncestor checks if the specified chain end links to a known
...
func (api *ConsensusAPI) checkInvalidAncestor(check common.Hash, head common.Hash) *engine.PayloadStatusV1 {
	...
	if header := api.unc.BlockChain().GetHeader(invalid.ParentHash, invalid.Number.Uint64()-1); header != nil && header.Difficulty.Sign() != 0 {
...
func (api *ConsensusAPI) heartbeat() {
	...
	if api.unc.BlockChain().Config().TerminalTotalDifficulty == nil {
		return
	}
	...
	for {
		...
		if api.unc.BlockChain().Config().TerminalTotalDifficultyPassed || api.unc.Merger().TDDReached() {
...
func (api *ConsensusAPI) GetPayloadBodiesByHashV1(hashes []common.Hash) []*engine.ExecutionPayloadBodyV1 {
	...
	for i, hash := range hashes {
		block := api.unc.BlockChain().GetBlockByHash(hash)
...
func (api *ConsensusAPI) GetPayloadBodiesByRangeV1(start, count hexutil.Uint64) ([]*engine.ExecutionPayloadBodyV1, error) {
	...
	current := api.unc.BlockChain().CurrentBlock().Number.Uint64()
	...
	for i := uint64(start); i <= last; i++ {
		block := api.unc.BlockChain().GetBlockByNumber(i)
