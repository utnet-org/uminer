...
func NewSimulatedBeacon(period uint64, unc *unc.Utility) (*SimulatedBeacon, error) {
	chainConfig := unc.APIBackend.ChainConfig()
	if !chainConfig.IsDevMode {
		return nil, errors.New("incompatible pre-existing chain configuration")
	}
	block := unc.BlockChain().CurrentBlock()
	current := engine.ForkchoiceStateV1{
		HeadBlockHash:      block.Hash(),
		SafeBlockHash:      block.Hash(),
		FinalizedBlockHash: block.Hash(),
	}
	engineAPI := newConsensusAPIWithoutHeartbeat(unc)
...
func (c *SimulatedBeacon) sealBlock(withdrawals []*types.Withdrawal) error {
	...
	if header := c.unc.BlockChain().CurrentBlock(); c.curForkchoiceState.HeadBlockHash != header.Hash() {
...
func (c *SimulatedBeacon) loopOnDemand() {
	var (
		...
		sub    = c.unc.TxPool().SubscribeTransactions(newTxs, true)
	)
...
func (c *SimulatedBeacon) finalizedBlockHash(number uint64) *common.Hash {
	...
	if finalizedBlock := c.unc.BlockChain().GetBlockByNumber(finalizedNumber); finalizedBlock != nil {
