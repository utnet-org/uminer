...
// support all the operations needed by the Utility chain protocols.
type txPool interface {
	...
	// can decide if to receive notifications only for newly seen transactions
...
type handlerConfig struct {
	...
	Sync           downloader.SyncMode    // If to snap or full sync
...
// newHandler returns a handler for all Utility chain management protocol.
...
// runEthPeer registers an unc peer into the joint unc/snap peerset, adds it to
// various subsystems and starts handling messages.
func (h *handler) runEthPeer(peer *unc.Peer, handler unc.Handler) error {
	if !h.incHandlers() {
	...
	if err := peer.Handshake(h.networkID, td, hash, genesis.Hash(), forkID, h.forkFilter); err != nil {
		peer.Log().Debug("Utility handshake failed", "err", err)
		return err
	}
	...
	peer.Log().Debug("Utility peer connected", "name", peer.Name())

	// Register the peer locally
	if err := h.peers.registerPeer(peer, snap); err != nil {
		peer.Log().Error("Utility peer registration failed", "err", err)
		return err
	}
	...
	if err := h.downloader.RegisterPeer(peer.ID(), peer.Version(), peer); err != nil {
		peer.Log().Error("Failed to register peer in unc syncer", "err", err)
		return err
	}
	if snap != nil {
	...
	for number, hash := range h.requiredBlocks {
		...
		go func(number uint64, hash common.Hash, req *unc.Request) {
			...
			select {
			case res := <-resCh:
				headers := ([]*types.Header)(*res.Res.(*unc.BlockHeadersRequest))
				if len(headers) == 0 {
...
// runSnapExtension registers a `snap` peer into the joint unc/snap peerset and
// starts handling inbound messages. As `snap` is only a satellite protocol to
// `unc`, all subsystem registrations and lifecycle management will be done by
// the main `unc` handler to prevent strange races.
...
func (h *handler) unregisterPeer(id string) {
	...
	peer := h.peers.peer(id)
	if peer == nil {
		logger.Error("Utility peer removal failed", "err", errPeerNotRegistered)
		return
	}
	// Remove the `unc` peer if it exists
	logger.Debug("Removing Utility peer", "snap", peer.snapExt != nil)
	...
	if err := h.peers.unregisterPeer(id); err != nil {
		logger.Error("Utility peer removal failed", "err", err)
	}
}
...
func (h *handler) Stop() {
	...
	log.Info("Utility protocol stopped")
}
