...
Use `devp2p enrdump <base64>` to verify and display an Utility Node Record.
...
- `-unc-network <mainnet/goerli/sepolia/holesky>` filters nodes by "unc" ENR entry
- `-les-server` filters nodes by LES server support
- `-snap` filters nodes by snap protocol support

For example, given a node set in `nodes.json`, you could create a filtered set containing
up to 20 unc mainnet nodes which also support snap sync using this command:

    devp2p nodeset filter nodes.json -unc-network mainnet -snap -limit 20
...
### Unc Protocol Test Suite

The Unc Protocol test suite is a conformance test suite for the [unc protocol][unc].

To run the unc protocol test suite against your implementation, the node needs to be initialized as such:

1. initialize the gunc node with the `genesis.json` file contained in the `testdata` directory
2. import the `halfchain.rlp` file in the `testdata` directory
3. run gunc with the following flags:
```
gunc --datadir <datadir> --nodiscover --nat=none --networkid 19763 --verbosity 5
```
...
The Eth66 test suite is also a conformance test suite for the unc 66 protocol version specifically.
To run the eth66 protocol test suite, initialize a gunc node as described above and run the following command,
