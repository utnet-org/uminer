...
loop:
	...
	// make sure unc protocol version is set for negotiation
	if c.negotiatedProtoVersion == 0 {
		return nil, errors.New("unc protocol version must be set in Conn")
	}
	if status == nil {
...
func (c *Conn) readAndServe(chain *Chain, timeout time.Duration) Message {
	...
	for time.Since(start) < timeout {
		...
		case *GetBlockHeaders:
			...
			resp := &BlockHeaders{
				RequestId:           msg.ReqID(),
				BlockHeadersRequest: unc.BlockHeadersRequest(headers),
			}
...
// headersMatch returns if the received headers match the given request
...
func (s *Suite) waitForBlockImport(conn *Conn, block *types.Block) error {
	...
	req := &GetBlockHeaders{
		GetBlockHeadersRequest: &unc.GetBlockHeadersRequest{
			Origin: unc.HashOrNumber{Hash: block.Hash()},
			Amount: 1,
		},
	}
...
func (s *Suite) hashAnnounce() error {
	...
	err = sendConn.Write(&BlockHeaders{
		...
		BlockHeadersRequest: unc.BlockHeadersRequest{nextBlock.Header()},
	})
	if err != nil {
		return fmt.Errorf("failed to write to connection: %v", err)
	}
