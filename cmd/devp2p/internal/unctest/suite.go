...
// NewSuite creates and returns a new unc-test suite that can
...
// a status message with it on the unc protocol.
...
// an unc `GetBlockHeaders` request and that the response is accurate.
func (s *Suite) TestGetBlockHeaders(t *utesting.T) {
	...
	req := &GetBlockHeaders{
		GetBlockHeadersRequest: &unc.GetBlockHeadersRequest{
			Origin:  unc.HashOrNumber{Hash: s.chain.blocks[1].Hash()},
			Amount:  2,
			Skip:    1,
			Reverse: false,
		},
...
func (s *Suite) TestZeroRequestID(t *utesting.T) {
	...
	req := &GetBlockHeaders{
		GetBlockHeadersRequest: &unc.GetBlockHeadersRequest{
			Origin: unc.HashOrNumber{Number: 0},
			Amount: 2,
		},
	}
...
func (s *Suite) TestGetBlockBodies(t *utesting.T) {
	...
	req := &GetBlockBodies{
		RequestId: uint64(55),
		GetBlockBodiesRequest: unc.GetBlockBodiesRequest{
...
// TestMaliciousTx sends several invalid transactions and tests if
...
// TestLargeTxRequest tests if a node can fulfill a large GetPooledTransactions
// request.
...
// TestNewPooledTxs tests if a node will do a GetPooledTransactions
...
func (s *Suite) TestNewPooledTxs(t *utesting.T) {
	...
	if conn.negotiatedProtoVersion < unc.ETH68 {
