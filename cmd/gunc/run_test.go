...
func init() {
	// Run the app if we've been exec'd as "gunc-test" in runGeth.
	reexec.Register("gunc-test", func() {
...
// spawns gunc with the given command line args. If the args don't set --datadir, the
...
func runGeth(t *testing.T, args ...string) *testgeth {
	...
	for i, arg := range args {
		...
		case "--miner.unicrpytbase":
			if i < len(args)-1 {
				tt.Unicrpytbase = args[i+1]
			}
		}
	}
	if tt.Datadir == "" {
	...
	// Boot "gunc". This actually runs the test binary but the TestMain
	// function will prevent any tests from running.
	tt.Run("gunc-test", args...)
