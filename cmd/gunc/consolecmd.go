...
import (
	...
	"github.com/yanhuangpai/go-utility/console"
...
var (
	...
	consoleCommand = &cli.Command{
		...
		Description: `
The Gunc console is an interactive shell for the JavaScript runtime environment
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://gunc.utility.org/docs/interacting-with-gunc/javascript-console.`,
	}
...
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://gunc.utility.org/docs/interacting-with-gunc/javascript-console.
This command allows to open a console on a running gunc node.`,
	}
...
JavaScript API. See https://gunc.utility.org/docs/interacting-with-gunc/javascript-console`,
	}
)

// localConsole starts a new gunc node, attaching a JavaScript console to it at the
// same time.
...
// remoteConsole will connect to a remote gunc instance, attaching a JavaScript
// console to it.
func remoteConsole(ctx *cli.Context) error {
	...
	if err != nil {
		utils.Fatalf("Unable to attach to remote gunc: %v", err)
	}
	config := console.Config{
		DataDir: utils.MakeDataDir(ctx),
		DocRoot: ctx.String(utils.JSpathFlag.Name),
...
// ephemeralConsole starts a new gunc node, attaches an ephemeral JavaScript
