...
This test shows how the `uvm t8n` can be used to calculate the (ethash) difficulty, if none is provided by the caller, 
...
```
[user@work uvm]$ ./uvm t8n --input.alloc=./testdata/19/alloc.json --input.txs=./testdata/19/txs.json --input.env=./testdata/19/env.json --output.result=stdout --state.fork=GrayGlacier
