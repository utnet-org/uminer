...
// Package core implements the Utility consensus protocol.
package core
...
import (
	...
	"github.com/yanhuangpai/go-utility/internal/syncx"
...
type CacheConfig struct {
	...
	TrieDirtyDisabled   bool          // If to disable trie write caching and GC completely (archive node)
	TrieTimeLimit       time.Duration // Time limit after which to flush the current in-memory trie to disk
	SnapshotLimit       int           // Memory allowance (MB) to use for caching snapshot entries in memory
	Preimages           bool          // If to store preimage of trie key to the disk
	StateHistory        uint64        // Number of blocks from head whose state histories are reserved.
	StateScheme         string        // Scheme used to store utility states and merkle tree nodes on top

	SnapshotNoBuild bool // If the background generation is allowed
...
type BlockChain struct {
	...
	db            uncdb.Database                   // Low level persistent database to store final content in
...
// available in the database. It initialises the default Utility Validator
// and Processor.
func NewBlockChain(db uncdb.Database, cacheConfig *CacheConfig, genesis *Genesis, overrides *ChainOverrides, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(header *types.Header) bool, txLookupLimit *uint64) (*BlockChain, error) {
	if cacheConfig == nil {
	...
	// If Gunc is initialized with an external ancient store, re-initialize the
...
// the given timestamp. Depending on if the node was snap synced or full
...
// persistent disk layer. Depending on if the node was snap synced or full, and
...
func (bc *BlockChain) setHeadBeyondRoot(head uint64, time uint64, root common.Hash, repair bool) (uint64, error) {
	...
	updateFn := func(db uncdb.KeyValueWriter, header *types.Header) (*types.Header, bool) {
	...
	delFn := func(db uncdb.KeyValueWriter, hash common.Hash, num uint64) {
	...
	// touching the header chain completely, unless the freezer is broken
	if repair {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		//
		// But if the `TxIndexTail` is not nil, e.g. Gunc is initialized with
		...
		for i, block := range blockChain {
			...
			if batch.ValueSize() > uncdb.IdealBatchSize || i == len(blockChain)-1 {
...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB) error {
	...
	if nodes > limit || imgs > 4*1024*1024 {
		bc.triedb.Cap(limit - uncdb.IdealBatchSize)
	}
...
// it's not checked in the Gunc side anymore.
...
func (bc *BlockChain) insertChain(chain types.Blocks, setHead bool) (int, error) {
	...
	for ; block != nil && err == nil || errors.Is(err, ErrKnownBlock); block, err = it.next() {
		...
		blockExecutionTimer.Update(ptime - trieRead)                    // The time spent on UVM processing
...
// Gunc will automatically construct the missing indices or delete the extra
// indices.
