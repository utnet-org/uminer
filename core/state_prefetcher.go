...
// Prefetch processes the state changes according to the Utility rules by running
...
func (p *statePrefetcher) Prefetch(block *types.Block, statedb *state.StateDB, cfg vm.Config, interrupt *atomic.Bool) {
	var (
		...
		uvm          = vm.NewEVM(blockContext, vm.TxContext{}, statedb, p.config, cfg)
	...
	for i, tx := range block.Transactions() {
		...
		if err := precacheTransaction(msg, p.config, gaspool, statedb, header, uvm); err != nil {
...
func precacheTransaction(msg *Message, config *params.ChainConfig, gaspool *GasPool, statedb *state.StateDB, header *types.Header, uvm *vm.UVM) error {
	// Update the uvm with the new transaction context.
	uvm.Reset(NewEVMTxContext(msg), statedb)
	// Add addresses to access list if applicable
	_, err := ApplyMessage(uvm, msg, gaspool)
	return err
}
