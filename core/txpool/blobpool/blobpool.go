...
//     commit to their own current state, which is independent of Utility mainnet
...
//     priority will be grouped jointly.
//
...
// Filter returns if the given transaction can be consumed by the blob pool.
...
// Has returns an indicator if subpool has a transaction cached with the
// given hash.
