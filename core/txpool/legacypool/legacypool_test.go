...
func (c *testChain) State() (*state.StateDB, error) {
	...
	if *c.trigger {
		...
		c.statedb.SetBalance(c.address, new(big.Int).SetUint64(params.Unicrpyt))
		*c.trigger = false
	}
	return stdb, nil
}
...
func TestStateChangeDuringReset(t *testing.T) {
	...
	statedb.SetBalance(address, new(big.Int).SetUint64(params.Unicrpyt))
...
// Tests if highest fee cap transaction is retained after a batch of high effective
