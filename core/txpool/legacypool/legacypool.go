...
// Package legacypool implements the normal UVM execution transaction pool.
package legacypool
...
// Filter returns if the given transaction can be consumed by the legacy
// pool, specifically, if it is a Legacy, AccessList or Dynamic transaction.
...
// validateTxBasics checks if a transaction is valid according to the consensus
...
// isGapped reports if the given transaction is immediately executable.
