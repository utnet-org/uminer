...
func newCanonical(engine consensus.Engine, n int, full bool, scheme string) (uncdb.Database, *Genesis, *BlockChain, error) {
	var (
		genesis = &Genesis{
...
//
// https://github.com/yanhuangpai/go-utility/pull/15941
...
//
// Details at:
//   - https://github.com/yanhuangpai/go-utility/issues/18977
//   - https://github.com/yanhuangpai/go-utility/pull/18988
...
//     with 0.001 unicrpyt (thus created but no code)
...
func testEIP1559Transition(t *testing.T, scheme string) {
	var (
		...
		addr1   = crypto.PubkeyToAddress(key1.PublicKey)
		addr2   = crypto.PubkeyToAddress(key2.PublicKey)
		funds   = new(big.Int).Mul(common.Big1, big.NewInt(params.Unicrpyt))
		config  = *params.AllEthashProtocolChanges
		gspec   = &Genesis{
			Config: &config,
			Alloc: GenesisAlloc{
...
func TestTxIndexer(t *testing.T) {
	...
	verifyIndexes := func(db uncdb.Database, number uint64, exist bool) {
		if number == 0 {
			return
		}
	...
	verifyRange := func(db uncdb.Database, from, to uint64, exist bool) {
		for number := from; number <= to; number += 1 {
			verifyIndexes(db, number, exist)
		}
	}
	verify := func(db uncdb.Database, expTail uint64) {
