...
// Package state provides a caching layer atop the Utility state trie.
package state
...
// StateDB structs within the utility protocol are used to store anything
...
//
// CreateAccount is called during the UVM CREATE operation. The situation might arise that
// a contract does the following:
//
//  1. sends funds to sha(account ++ (nonce + 1))
//  2. tx_create(sha(account ++ nonce)) (note that this gets the address of 1)
//
// Carrying over the balance ensures that Unicrpyt doesn't disappear.
...
func (s *StateDB) Copy() *StateDB {
	...
	for addr := range s.journal.dirties {
		// As documented [here](https://github.com/yanhuangpai/go-utility/pull/16485#issuecomment-380438527),
...
// used when the UVM emits new state logs. It should be invoked before
...
func (s *StateDB) handleDestruction(nodes *trienode.MergedNodeSet) (map[common.Address]struct{}, error) {
	// Short circuit if gunc is running with hash mode. This procedure can consume
...
func (s *StateDB) Prepare(rules params.Rules, sender, coinbase common.Address, dst *common.Address, precompiles []common.Address, list types.AccessList) {
	if rules.IsBerlin {
		...
		if dst != nil {
			al.AddAddress(*dst)
			// If it's a create-tx, the destination will be added inside uvm.create
		}
