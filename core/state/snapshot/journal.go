...
type journalGenerator struct {
	// Indicator that if the database was in progress of being wiped.
...
func loadAndParseJournal(db uncdb.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
	...
	// not existent, e.g. the disk layer is generating while that the Gunc
...
func loadSnapshot(diskdb uncdb.KeyValueStore, triedb *trie.Database, root common.Hash, cache int, recovery bool, noBuild bool) (snapshot, bool, error) {
	...
	//
	// Possible scenario: Gunc was crashed without persisting journal and then
...
func iterateJournal(db uncdb.KeyValueReader, callback journalCallback) error {
