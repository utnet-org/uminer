...
type Matcher struct {
	...
	running atomic.Bool // Atomic flag if a session is live or not
}
...
// that address/topic, and binary AND-ing those vectors jointly.
	...
	go func() {
		...
		for {
			...
			case subres, ok := <-process:
				...
				// Gather all the sub-results and merge them jointly
				var orVector []byte
...
// the requested retrieval queue to be serviced jointly with other sessions.
//
