...
type ContractCaller interface {
	...
	// CallContract executes an Utility contract call with the specified data as the
	// input.
	CallContract(ctx context.Context, call utility.CallMsg, blockNumber *big.Int) ([]byte, error)
}
...
type PendingContractCaller interface {
	...
	// PendingCallContract executes an Utility contract call against the pending state.
	PendingCallContract(ctx context.Context, call utility.CallMsg) ([]byte, error)
}
...
type BlockHashContractCaller interface {
	...
	// CallContractAtHash executes an Utility contract all against the state at the specified block hash.
	CallContractAtHash(ctx context.Context, call utility.CallMsg, blockHash common.Hash) ([]byte, error)
}
...
type ContractTransactor interface {
	...
	EstimateGas(ctx context.Context, call utility.CallMsg) (gas uint64, err error)
...
type ContractFilterer interface {
	...
	FilterLogs(ctx context.Context, query utility.FilterQuery) ([]types.Log, error)

	// SubscribeFilterLogs creates a background log filtering operation, returning
	// a subscription immediately, which can be used to stream the found events.
	SubscribeFilterLogs(ctx context.Context, query utility.FilterQuery, ch chan<- types.Log) (utility.Subscription, error)
}
