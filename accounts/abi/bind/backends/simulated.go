...
import (
	...
	utility "github.com/yanhuangpai/go-utility"
...
type SimulatedBackend struct {
	database   uncdb.Database   // In memory database to store our testing data
	blockchain *core.BlockChain // Utility blockchain to handle the consensus
...
func NewSimulatedBackendWithDatabase(database uncdb.Database, alloc core.GenesisAlloc, gasLimit uint64) *SimulatedBackend {
...
// revertError is an API error that encompasses an UVM revert with JSON error
...
func (b *SimulatedBackend) CallContract(ctx context.Context, call utility.CallMsg, blockNumber *big.Int) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) CallContractAtHash(ctx context.Context, call utility.CallMsg, blockHash common.Hash) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) callContractAtHead(ctx context.Context, call utility.CallMsg) ([]byte, error) {
...
func (b *SimulatedBackend) PendingCallContract(ctx context.Context, call utility.CallMsg) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) EstimateGas(ctx context.Context, call utility.CallMsg) (uint64, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) callContract(ctx context.Context, call utility.CallMsg, header *types.Header, stateDB *state.StateDB) (*core.ExecutionResult, error) {
	...
	if !b.blockchain.Config().IsLondon(header.Number) {
		...
		if call.GasPrice != nil {
			...
			// Backfill the legacy gasPrice for UVM execution, unless we're all zeroes
			call.GasPrice = new(big.Int)
...
func (b *SimulatedBackend) FilterLogs(ctx context.Context, query utility.FilterQuery) ([]types.Log, error) {
...
func (b *SimulatedBackend) SubscribeFilterLogs(ctx context.Context, query utility.FilterQuery, ch chan<- types.Log) (utility.Subscription, error) {
...
func (b *SimulatedBackend) SubscribeNewHead(ctx context.Context, ch chan<- *types.Header) (utility.Subscription, error) {
...
func (fb *filterBackend) ChainDb() uncdb.Database { return fb.db }
