...
// Package accounts implements high level Utility account management.
package accounts
...
// Account represents an Utility account located at a specific location defined
// by the optional URL field.
type Account struct {
	Address common.Address `json:"address"` // Utility account address derived from the key
...
type Wallet interface {
	...
	// chain state reader.
	SelfDerive(bases []DerivationPath, chain utility.ChainStateReader)
...
//
//	keccak256("\x19Unitily Signed Message:\n"${message length}${message}).
//
...
// The hash is calculated as
//
//	keccak256("\x19UnitilySigned Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func TextAndHash(data []byte) ([]byte, string) {
	msg := fmt.Sprintf("\x19Unitily Signed Message:\n%d%s", len(data), string(data))
