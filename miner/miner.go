...
// Package miner implements Utility block creation and mining.
package miner
...
type Config struct {
	Unicrpytbase common.Address `toml:",omitempty"` // Public address for block mining rewards
...
var DefaultConfig = Config{
	...
	// for payload generation. It should be enough for Gunc to
	// run 3 rounds.
...
func New(unc Backend, config *Config, chainConfig *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine, isLocalBlock func(header *types.Header) bool) *Miner {
	miner := &Miner{
		mux:     mux,
		unc:     unc,
		engine:  engine,
		...
		worker:  newWorker(config, chainConfig, engine, unc, mux, isLocalBlock, true),
	}
	miner.wg.Add(1)
	go miner.update()
	return miner
...
func (miner *Miner) SetUnicrpytbase(addr common.Address) {
	miner.worker.setUnicrpytbase(addr)
}
