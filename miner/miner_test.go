...
// TestMinerSetUnicrpytbase checks that unicrpytbase becomes set even if mining isn't
// possible at the moment
func TestMinerSetUnicrpytbase(t *testing.T) {
	miner, mux, cleanup := createMiner(t)
	defer cleanup(false)
	miner.Start()
	waitForMiningState(t, miner, true)
	...
	miner.SetUnicrpytbase(coinbase)
	if addr := miner.worker.unicrpytbase(); addr != coinbase {
		t.Fatalf("Unexpected unicrpytbase want %x got %x", coinbase, addr)
	}
}
...
func createMiner(t *testing.T) (*Miner, *event.TypeMux, func(skipMiner bool)) {
	...
	config := Config{
		Unicrpytbase: common.HexToAddress("123456789"),
	}
	// Create chainConfig
