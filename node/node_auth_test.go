...
func TestAuthEndpoints(t *testing.T) {
	...
	// Gunc must read it from a file, and does not support in-memory JWT secrets, so we create a temporary file.
	...
	conf := &Config{
		...
		WSModules:   []string{"unc", "engine"},
		HTTPModules: []string{"unc", "engine"},
	}
	node, err := New(conf)
	if err != nil {
