...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gunc is "gunc". If no
...
// These resources are resolved differently for "gunc" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gunc 1.4 are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn && !c.oldGethResourceWarning {
				...
				log.Warn("Using deprecated resource file, please move this file to the 'gunc' subdirectory of datadir.", "file", oldpath)
			}
			return oldpath
		}
	}
...
func (c *Config) GetKeyStoreDir() (string, bool, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = os.MkdirTemp("", "go-utility-keystore")
		isEphemeral = true
	}
