...
type Type struct {
	Name      string
	Kind      reflect.Kind
	IsEncoder bool  // if type implements rlp.Encoder
	IsDecoder bool  // if type implements rlp.Decoder
	Elem      *Type // non-nil for Kind values of Ptr, Slice, Array
}

// DefaultNilValue determines if a nil pointer to t encodes/decodes
...
type Tags struct {
	...
	// rlp:"tail" controls if this field swallows additional list elements. It can
