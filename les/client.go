...
// Package les implements the Light Utility Subprotocol.
package les
...
func New(stack *node.Node, config *uncconfig.Config) (*LightUnility, error) {
	...
	lesDb, err := stack.OpenDatabase("les.client", 0, 0, "unc/db/lesclient/", false)
	if err != nil {
		return nil, err
	}
...
func (s *LightDummyAPI) Unicrpytbase() (common.Address, error) {
...
func (s *LightUnility) Stop() error {
	...
	s.lesDb.Close()
	s.wg.Wait()
	log.Info("Light utility stopped")
	return nil
}
