...
func (b *LesApiBackend) HeaderByNumber(ctx context.Context, number rpc.BlockNumber) (*types.Header, error) {
	...
	if number == rpc.PendingBlockNumber {
		return b.unc.blockchain.CurrentHeader(), nil
	}
	if number == rpc.LatestBlockNumber {
		return b.unc.blockchain.CurrentHeader(), nil
	}
	return b.unc.blockchain.GetHeaderByNumberOdr(ctx, uint64(number))
}
...
func (b *LesApiBackend) BlockByHash(ctx context.Context, hash common.Hash) (*types.Block, error) {
	return b.unc.blockchain.GetBlockByHash(ctx, hash)
}
...
func (b *LesApiBackend) BlockByNumberOrHash(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash) (*types.Block, error) {
	...
	if hash, ok := blockNrOrHash.Hash(); ok {
		...
		if blockNrOrHash.RequireCanonical && b.unc.blockchain.GetCanonicalHash(block.NumberU64()) != hash {
			return nil, errors.New("hash is not currently canonical")
		}
		return block, nil
	}
...
func (b *LesApiBackend) GetBody(ctx context.Context, hash common.Hash, number rpc.BlockNumber) (*types.Body, error) {
	return light.GetBody(ctx, b.unc.odr, hash, uint64(number))
}
...
func (b *LesApiBackend) StateAndHeaderByNumber(ctx context.Context, number rpc.BlockNumber) (*state.StateDB, *types.Header, error) {
	...
	return light.NewState(ctx, header, b.unc.odr), header, nil
}
...
func (b *LesApiBackend) GetReceipts(ctx context.Context, hash common.Hash) (types.Receipts, error) {
	if number := rawdb.ReadHeaderNumber(b.unc.chainDb, hash); number != nil {
		return light.GetBlockReceipts(ctx, b.unc.odr, hash, *number)
	}
	return nil, nil
}

func (b *LesApiBackend) GetLogs(ctx context.Context, hash common.Hash, number uint64) ([][]*types.Log, error) {
	return light.GetBlockLogs(ctx, b.unc.odr, hash, number)
}

func (b *LesApiBackend) GetTd(ctx context.Context, hash common.Hash) *big.Int {
	if number := rawdb.ReadHeaderNumber(b.unc.chainDb, hash); number != nil {
		return b.unc.blockchain.GetTdOdr(ctx, hash, *number)
	}
	return nil
}

func (b *LesApiBackend) GetEVM(ctx context.Context, msg *core.Message, state *state.StateDB, header *types.Header, vmConfig *vm.Config, blockCtx *vm.BlockContext) (*vm.UVM, func() error) {
	if vmConfig == nil {
	...
	return vm.NewEVM(context, txContext, state, b.unc.chainConfig, *vmConfig), state.Error
}

func (b *LesApiBackend) SendTx(ctx context.Context, signedTx *types.Transaction) error {
	return b.unc.txPool.Add(ctx, signedTx)
}

func (b *LesApiBackend) RemoveTx(txHash common.Hash) {
	b.unc.txPool.RemoveTx(txHash)
}

func (b *LesApiBackend) GetPoolTransactions() (types.Transactions, error) {
	return b.unc.txPool.GetTransactions()
}

func (b *LesApiBackend) GetPoolTransaction(txHash common.Hash) *types.Transaction {
	return b.unc.txPool.GetTransaction(txHash)
}

func (b *LesApiBackend) GetTransaction(ctx context.Context, txHash common.Hash) (*types.Transaction, common.Hash, uint64, uint64, error) {
	return light.GetTransaction(ctx, b.unc.odr, txHash)
}

func (b *LesApiBackend) GetPoolNonce(ctx context.Context, addr common.Address) (uint64, error) {
	return b.unc.txPool.GetNonce(ctx, addr)
}

func (b *LesApiBackend) Stats() (pending int, queued int) {
	return b.unc.txPool.Stats(), 0
}
...
func (b *LesApiBackend) SubscribeNewTxsEvent(ch chan<- core.NewTxsEvent) event.Subscription {
	return b.unc.txPool.SubscribeNewTxsEvent(ch)
}
...
func (b *LesApiBackend) SyncProgress() utility.SyncProgress {
	return utility.SyncProgress{}
}

func (b *LesApiBackend) ProtocolVersion() int {
	return b.unc.LesVersion() + 10000
}
...
func (b *LesApiBackend) ChainDb() uncdb.Database {
...
func (b *LesApiBackend) StateAtBlock(ctx context.Context, block *types.Block, reexec uint64, base *state.StateDB, readOnly bool, preferDisk bool) (*state.StateDB, tracers.StateReleaseFunc, error) {
	return b.unc.stateAtBlock(ctx, block, reexec)
}
