...
static void bm_smi_get_attr(bm_handle_t handle, int bmctl_fd, int dev_id) {
    ...
    if (!status) {
        ...
        g_attr[dev_id].mem_used += (int)(g_attr[dev_id].vpu_mem_used_size[0] / 1024 / 1024);
        g_attr[dev_id].mem_total += (int)(g_attr[dev_id].vpu_mem_total_size[0] / 1024 / 1024);
#endif
    }
#ifdef SOC_MODE
...
static void bm_smi_maxclk_to_str(int dev_id, char *s) {
    if (g_attr[dev_id].tpu_max_clock == ATTR_NOTSUPPORTED_VALUE) {
        ...
        snprintf(s, 5, "%dM", g_attr[dev_id].tpu_max_clock);
    }
}
...
static void bm_smi_currclk_to_str(int dev_id, char *s) {
    if (g_attr[dev_id].tpu_current_clock == ATTR_NOTSUPPORTED_VALUE) {
        ...
        snprintf(s, 5, "%dM", g_attr[dev_id].tpu_current_clock);
    }
}
...
static void bm_smi_print_text_info(HANDLE bmctl_device, int start_dev, int last_dev) {
    ...
    for (i = start_dev; i < (last_dev + 1); i++) {
        ...
        printf("%dMB ", g_attr[i].mem_total);
    }

    bm_smi_boardp_to_str(i - 1, boardp_s);
    bm_smi_12v_curr_to_str(i - 1, c12v_s);
    bm_smi_sn_to_str(i - 1, sn_s);
    bm_smi_maxpower_to_str(i - 1, maxp_s);
    bm_smi_fan_to_str(i - 1, fan_s);

    printf("board total info: %s ", c12v_s);
    printf("%s ", sn_s);
    printf("%s ", maxp_s);
    printf("%s ", boardp_s);
    printf("%s ", fan_s);
    board_avg_temp = board_avg_temp / (last_dev - start_dev + 1);
    printf("%dC %dC\n", board_max_temp, board_avg_temp);
}
#endif
