...
int bm1682_update_flash(bm_handle_t handle, Bin_buffer *itcm_buf,
        Bin_buffer *ddr_buf, Bin_buffer *bin_buf) {
  #ifdef __linux__
  pthread_t print_thread;
  bm_status_t ret;
  bm_device_mem_t flash_device_mem;

  /* download flash file */
  flash_device_mem = bm_mem_from_device(0x100f00000, bin_buf->size);
  ret = bm_memcpy_s2d(handle, flash_device_mem, bin_buf->buf);
  if (ret != BM_SUCCESS) {
    printf("download flash bin failed!\n");
    return BM_ERR_FAILURE;
  }
  printf("flash bin downloaded.\n");

  /* update firmware */
  bm_update_fw(handle, itcm_buf, ddr_buf);

  /* start programming flash */
  bm_send_api(handle, BM_API_ID_FLASH_UPDATE, NULL, 0);

  pthread_create(&print_thread, NULL, print_wait, NULL);
...
static int validate_firmware_and_board_type(bm_handle_t handle, Bin_buffer *bin_buf) {
  ...
  strncpy(board_type, board_name+5, 10);
  if (ret != BM_SUCCESS) {
    return -1;
  }
  printf("board type=%s\n", board_type);
...
bool is_valid_mcu(Bin_buffer *bin_buf) {
  unsigned char * const image = bin_buf->buf;
  unsigned long size = bin_buf->size;
...
int main(int argc, char *argv[]) {
  ...
  for (i = chip_id; i < chip_id + count; i++) {
    ...
    if (handle->misc_info.chipid == 0x1682) {
      ...
      bm1682_update_flash(handle, &itcm_buf, &ddr_buf, &bin_buf);
