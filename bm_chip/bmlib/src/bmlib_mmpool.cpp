...
#ifdef MEM_POOL_DEBUG
  pirntf("mem_pool: alloc addr 0x%llx with size of %lld; \n", addr_to_alloc,
          size_to_alloc);
  pirntf("mem_pool: actual size required = %lld\n", size);
#endif
...
#ifdef MEM_POOL_DEBUG
  printf("mem_pool_free: addr_to_free = 0x%llx; size_to_free = %lld\n",
