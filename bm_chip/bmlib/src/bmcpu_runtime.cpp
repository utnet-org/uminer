...
bm_status_t bmcpu_gen_aes_key(bm_handle_t handle) {
    u64               api_handle;
    u64               api_data;
    bm_aeskey_t       api_aes_key;
    int               ret;

    ret = bm_send_api_ext(handle,
                          BM_API_ID_GEN_AES_KEY,
                          (const u8 *)&api_aes_key,
                          sizeof(bm_aeskey_t),
                          &api_handle);
    if (ret != BM_SUCCESS) {
        bmlib_log(BMCPU_RUNTIME_LOG_TAG,
                  BMLIB_LOG_ERROR,
                  "gen aes key send api error, ret %d\n",
                  ret);
        return BM_ERR_FAILURE;
    }

    ret = bm_query_api_data(handle, BM_API_ID_GEN_AES_KEY, api_handle, &api_data, 10000);
    if (ret != 0) {
        bmlib_log(BMCPU_RUNTIME_LOG_TAG,
                  BMLIB_LOG_ERROR,
                  "get aes key query api error, ret %d\n",
                  ret);
        return BM_ERR_FAILURE;
    }
    if((s64)api_data == -1){
        bmlib_log(BMCPU_RUNTIME_LOG_TAG,
                  BMLIB_LOG_ERROR,
                  "AES KEY FOUND IN EFUSE,REQUESET FAIL!!\n",
                  ret);
        return BM_ERR_FAILURE;
    }
    return BM_SUCCESS;                         
...
bm_status_t bmcpu_gen_p2_pubkey(bm_handle_t     handle,
                                unsigned char*  p2,
                                unsigned int*   size_p2,
    ...
    bm_device_mem_t   dev_mem;
    int               ret;
    bm_p2_pubkey_t   api_p2_pubkey;
    
    if (NULL == p2 ||  NULL == pubkey ){
        bmlib_log(BMCPU_RUNTIME_LOG_TAG,
                      BMLIB_LOG_ERROR,
                      "gen p2 public param error\n");
            return BM_ERR_PARAM;    
    }

    ret = bm_send_api_ext(handle,
                          BM_API_ID_GEN_P2_PUBKEY,
                          (const u8 *)&api_p2_pubkey,
                          sizeof(bm_p2_pubkey_t),
                          &api_handle);
    if (ret != BM_SUCCESS) {
        bmlib_log(BMCPU_RUNTIME_LOG_TAG,
                  BMLIB_LOG_ERROR,
                  "gen public key send api error, ret %d\n",
                  ret);
        return BM_ERR_FAILURE;
    }

    ret = bm_query_api_data(handle, BM_API_ID_GEN_P2_PUBKEY, api_handle, &api_data, 10000);
    if (ret != 0) {
        bmlib_log(BMCPU_RUNTIME_LOG_TAG,
                  BMLIB_LOG_ERROR,
                  "get public key query api error, ret %d\n",
                  ret);
        return BM_ERR_FAILURE;
    }
    if((s64)api_data == -10){
        bmlib_log(BMCPU_RUNTIME_LOG_TAG,
                  BMLIB_LOG_ERROR,
                  "AES KEY NOT FOUND IN EFUSE OR P2 GENERATED,REQUESET FAIL!\n",
                  ret);
        return BM_ERR_EFUSE;
    }
    if((s64)api_data < 0){
        bmlib_log(BMCPU_RUNTIME_LOG_TAG,
                  BMLIB_LOG_ERROR,
                  "bmcpu not generate p2 and pubkey  !\n",
                  ret);
        return BM_ERR_FAILURE;
    }
    *size_p2     = (u32)(api_data & 0xFFFFFFFF);
    *size_pubkey = (u32)(api_data >> 32);
    dev_mem = bm_mem_from_device(RSA_PUBLIC_KEY_ADDR, *size_pubkey);
    ret = bm_memcpy_d2s(handle, pubkey, dev_mem);
    if (ret != BM_SUCCESS) {
        bmlib_log(BMCPU_RUNTIME_LOG_TAG,
                  BMLIB_LOG_ERROR,
                  "get  public_key copy from device to system error, ret = %d\n",
                  ret);
        return BM_ERR_FAILURE;
    }
    unsigned int size_p2_padding = (*size_p2 + 15) & ~15;
    printf("[bmcpu_gen_p2_pubkey]size_p2 : %d size_pubkey: %d size_p2_padding :%d\n",\
     		*size_p2,*size_pubkey,size_p2_padding);
    dev_mem = bm_mem_from_device(RSA_PRIVATE_KEY_ADDR,size_p2_padding);
    ret = bm_memcpy_d2s(handle, p2, dev_mem);
    if (ret != BM_SUCCESS) {
        bmlib_log(BMCPU_RUNTIME_LOG_TAG,
                  BMLIB_LOG_ERROR,
                  "get p2_key copy from device to system error, ret = %d\n",
                  ret);
        return BM_ERR_FAILURE;
    }
...
bm_status_t bmcpu_gen_sign(bm_handle_t           handle,
                           unsigned char*        signature,
                           unsigned int*         size_signature,
                           unsigned char*        digest,
                           unsigned int          size_digest,
                           unsigned char*        p2,
                           unsigned int          size_p2){
    u64               api_handle;
    u64               api_data;
    bm_device_mem_t   dev_mem;
    int               ret;
    bm_p2_digest_t    api_p2_digest;
    if (NULL == p2 ||  NULL == digest 
        || size_digest > 0x10000 || size_p2 > 0x1000){
        bmlib_log(BMCPU_RUNTIME_LOG_TAG,
                      BMLIB_LOG_ERROR,
                      "gen sign param error\n");
            return BM_ERR_PARAM;    
    }
    dev_mem = bm_mem_from_device(RSA_DIGEST_ADDR , size_digest);
    ret = bm_memcpy_s2d(handle, dev_mem, digest);
    if (ret != BM_SUCCESS) {
        bmlib_log(BMCPU_RUNTIME_LOG_TAG,
                  BMLIB_LOG_ERROR,
                  "digest copy from system to device error, ret = %d\n",
                  ret);
        return BM_ERR_FAILURE;
    }

    unsigned int size_p2_padding = (size_p2 + 15) & ~15;
    printf("[bmcpu_gen_sign]size_p2 : %d size_digest: %d size_p2_padding :%d\n",\
     		size_p2, size_digest, size_p2_padding);
    dev_mem = bm_mem_from_device(RSA_PRIVATE_KEY_ADDR , size_p2_padding);
    ret = bm_memcpy_s2d(handle, dev_mem, p2);
    if (ret != BM_SUCCESS) {
        bmlib_log(BMCPU_RUNTIME_LOG_TAG,
                  BMLIB_LOG_ERROR,
                  "p2 copy from system to device error, ret = %d\n",
                  ret);
        return BM_ERR_FAILURE;
    }

    api_p2_digest.size_p2 = size_p2;
    api_p2_digest.size_digest = size_digest;
    ret = bm_send_api_ext(handle,
                          BM_API_ID_GEN_SIGN,
                          (const u8 *)&api_p2_digest,
                          sizeof(bm_p2_digest_t),
                          &api_handle);
    if (ret != BM_SUCCESS) {
        bmlib_log(BMCPU_RUNTIME_LOG_TAG,
                  BMLIB_LOG_ERROR,
                  "gen_sign send api error, ret %d\n",
                  ret);
        return BM_ERR_FAILURE;
    }                      
    ret = bm_query_api_data(handle, BM_API_ID_GEN_SIGN, api_handle, &api_data, 10000);
    if (ret != 0) {
        bmlib_log(BMCPU_RUNTIME_LOG_TAG,
                  BMLIB_LOG_ERROR,
                  "gen_sign query api error, ret %d\n",
                  ret);
        return BM_ERR_FAILURE;
    }
    if((s64)api_data < 0){
        bmlib_log(BMCPU_RUNTIME_LOG_TAG,
                  BMLIB_LOG_ERROR,
                  "bmcpu not generate signature  !\n",
                  ret);
        return BM_ERR_FAILURE;
    }

    *size_signature = (u32)(api_data & 0xFFFFFFFF);
    dev_mem = bm_mem_from_device(RSA_CIPHER_ADDR , size_p2);
    ret = bm_memcpy_d2s(handle,signature, dev_mem);
     if (ret != BM_SUCCESS) {
        bmlib_log(BMCPU_RUNTIME_LOG_TAG,
                  BMLIB_LOG_ERROR,
                  "signature copy from device to system error, ret = %d\n",
                  ret);
        return BM_ERR_FAILURE;
    }
