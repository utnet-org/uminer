...
int get_depthwise_deconv_secs_info(
  ...
  int bottom_n = input_shape.n;
  int c_per_NPU = ceiling_func_shift(bottom_c, NPU_SHIFT);

  int bias_tensor_size = using_bias ? c_per_NPU * FLOAT_SIZE : 0;
  int bank_size = LOCAL_MEM_SIZE / LOCAL_MEM_BANKS;

  int weight_tensor_size = c_per_NPU * get_neuron_csize_local(kh, kw);
  ...
  int weight_b_align = ALIGN(weight_tensor_size + bias_tensor_size, bank_size);
  if (weight_b_align > 2 * bank_size)
    return 0;
  int oneFeatureMem = of_csize_local;
  if (oneFeatureMem < if_csize_local) oneFeatureMem = if_csize_local;
  float inRatio = 1.0 / 2.0;
  int local_mem_inputbanks = (LOCAL_MEM_BANKS / 2) / 2;
  if (local_mem_inputbanks == 0) local_mem_inputbanks  = 1;
  float percentOneFeatureMem = (float)bank_size / (float)oneFeatureMem *
                               (float)local_mem_inputbanks;
  int n_step = 0;
  int c_step = 0;
  int h_step = 0;

  if (percentOneFeatureMem >= (bottom_n * c_per_NPU)) {
    n_step = bottom_n;
    c_step = bottom_c;
    h_step = top_h;
  } else if (percentOneFeatureMem >= c_per_NPU) {
    n_step = (int)(percentOneFeatureMem) / c_per_NPU;
    c_step = bottom_c;
    h_step = top_h;
  } else if (percentOneFeatureMem >= 1) {
    n_step = 1;
    c_step = (int)(percentOneFeatureMem) * NPU_NUM;
    h_step = top_h;
  } else {
    n_step = 1;
    c_step = NPU_NUM;
    int h_tmp = (int)(percentOneFeatureMem * top_h);
    if (get_neuron_csize_local(h_tmp, top_w) < local_mem_inputbanks * bank_size) {
      h_tmp -= 1;
    }
    if (h_tmp < kh_ext)
      return 0;
    int h_input_tmp = (h_tmp - kh_ext) / stride_h + 1;
    h_step = (h_input_tmp - 1) * stride_h + kh_ext;
  }
  secs_info->nsecs = n_step;
  secs_info->csecs = c_step;
  secs_info->hsecs = h_step;
  secs_info->Ratio = inRatio;
  return 1;
}
