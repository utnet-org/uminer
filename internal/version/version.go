...
const ourPath = "github.com/yanhuangpai/go-utility" // Path to our module
...
// conventions in the Utility p2p network.
...
//
// If the package that is currently executing is a prefixed by our go-utility
...
// version and if it was replaced by another module.
...
func versionInfo(info *debug.BuildInfo) string {
	// If the main package is from our repo, prefix version with "gunc".
	if strings.HasPrefix(info.Path, ourPath) {
		return fmt.Sprintf("gunc %s", info.Main.Version)
	}
	...
	if mod == nil {
		...
		// version.
		return version + fmt.Sprintf("gunc %s", params.VersionWithMeta)
	}
