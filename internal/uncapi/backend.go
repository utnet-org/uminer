...
// Package uncapi implements the general Utility API functions.
package uncapi
...
type Backend interface {
	// General Utility API
	SyncProgress() utility.SyncProgress
	...
	ExtRPCEnabled() bool
	RPCGasCap() uint64            // global gas cap for unc_call over rpc: DoS protection
	RPCEVMTimeout() time.Duration // global timeout for unc_call over rpc: DoS protection
	...
	GetEVM(ctx context.Context, msg *core.Message, state *state.StateDB, header *types.Header, vmConfig *vm.Config, blockCtx *vm.BlockContext) (*vm.UVM, func() error)
	...
	// unc/filters needs to be initialized from this backend type, so methods needed by
