...
import (
	...
	"github.com/yanhuangpai/go-utility/internal/blocktest"
...
func (b testBackend) SyncProgress() utility.SyncProgress { return utility.SyncProgress{} }
...
func (b testBackend) ChainDb() uncdb.Database           { return b.db }
...
func (b testBackend) GetEVM(ctx context.Context, msg *core.Message, state *state.StateDB, header *types.Header, vmConfig *vm.Config, blockContext *vm.BlockContext) (*vm.UVM, func() error) {
...
func TestEstimateGas(t *testing.T) {
	...
	var (
		...
		genesis  = &core.Genesis{
			...
			Alloc: core.GenesisAlloc{
				accounts[0].addr: {Balance: big.NewInt(params.Unicrpyt)},
				accounts[1].addr: {Balance: big.NewInt(params.Unicrpyt)},
			},
		}
		genBlocks      = 10
	...
	var testSuite = []struct {
		...
		},
		{
			...
			call:        TransactionArgs{},
			overrides: StateOverride{
				randomAccounts[0].addr: OverrideAccount{Balance: newRPCBalance(new(big.Int).Mul(big.NewInt(1), big.NewInt(params.Unicrpyt)))},
			},
			expectErr: nil,
			want:      53000,
		},
	...
	var (
		...
		genesis  = &core.Genesis{
			...
			Alloc: core.GenesisAlloc{
				accounts[0].addr: {Balance: big.NewInt(params.Unicrpyt)},
				accounts[1].addr: {Balance: big.NewInt(params.Unicrpyt)},
				accounts[2].addr: {Balance: big.NewInt(params.Unicrpyt)},
			},
		}
		genBlocks = 10
		signer    = types.HomesteadSigner{}
...
func TestRPCGetBlockOrHeader(t *testing.T) {
	...
	var (
		...
		genesis    = &core.Genesis{
			Config: params.TestChainConfig,
			Alloc: core.GenesisAlloc{
				acc1Addr: {Balance: big.NewInt(params.Unicrpyt)},
				acc2Addr: {Balance: big.NewInt(params.Unicrpyt)},
			},
		}
		genBlocks = 10
		signer    = types.HomesteadSigner{}
	...
	var (
		...
		genesis    = &core.Genesis{
			...
			Alloc: core.GenesisAlloc{
				...
				//     }
				// }
				contract: {Balance: big.NewInt(params.Unicrpyt), Code: common.FromHex("")},
			},
		}
...
func TestRPCGetTransactionReceipt(t *testing.T) {
	...
	for i, tt := range testSuite {
		...
		testRPCResponseWithFile(t, i, result, "unc_getTransactionReceipt", tt.file)
	}
}
...
func TestRPCGetBlockReceipts(t *testing.T) {
	...
	for i, tt := range testSuite {
		...
		testRPCResponseWithFile(t, i, result, "unc_getBlockReceipts", tt.file)
	}
}
