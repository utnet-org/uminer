...
//
// https://github.com/yanhuangpai/go-utility/wiki/Management-APIs#personal_sign
...
//
// https://github.com/yanhuangpai/go-utility/wiki/Management-APIs#personal_ecRecover
func (s *PersonalAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, errors.New("invalid Utility signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and gunc.
...
// BlockChainAPI provides an API to access Utility blockchain data.
type BlockChainAPI struct {
	b Backend
}

// NewBlockChainAPI creates a new Utility blockchain API.
func NewBlockChainAPI(b Backend) *BlockChainAPI {
	return &BlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current Utility chain config.
//
...
// proofList implements uncdb.KeyValueWriter and collects the proofs as
...
func doCall(ctx context.Context, b Backend, args TransactionArgs, state *state.StateDB, header *types.Header, overrides *StateOverride, blockOverrides *BlockOverrides, timeout time.Duration, globalGasCap uint64) (*core.ExecutionResult, error) {
	...
	uvm, vmError := b.GetEVM(ctx, msg, state, header, &vm.Config{NoBaseFee: true}, &blockCtx)

	// Wait for the context to be done and cancel the uvm. Even if the
	// UVM has finished, cancelling may be done (repeatedly)
	go func() {
		<-ctx.Done()
		uvm.Cancel()
	}()
	...
	result, err := core.ApplyMessage(uvm, msg, gp)
...
func DoCall(ctx context.Context, b Backend, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides *StateOverride, blockOverrides *BlockOverrides, timeout time.Duration, globalGasCap uint64) (*core.ExecutionResult, error) {
	defer func(start time.Time) { log.Debug("Executing UVM call finished", "runtime", time.Since(start)) }(time.Now())
...
// revertError is an API error that encompasses an UVM revertal with JSON error
...
//
// https://github.com/ethereum/wiki/wiki/JSON-RPC#unc_sign
...
// DebugAPI is the collection of Utility APIs exposed over the debugging
// namespace.
...
// Version returns the current utility protocol version.
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	feeEth := new(big.Float).Quo(new(big.Float).SetInt(new(big.Int).Mul(gasPrice, new(big.Int).SetUint64(gas))), new(big.Float).SetInt(big.NewInt(params.Unicrpyt)))
	feeFloat, _ := feeEth.Float64()
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f unicrpyt) exceeds the configured cap (%.2f unicrpyt)", feeFloat, cap)
	}
	return nil
}
