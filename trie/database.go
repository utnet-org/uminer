...
type Config struct {
	Preimages bool           // Flag if the preimage of node key is recorded
...
type backend interface {
	...
	// to disk. Report specifies if logs will be displayed in info level.
...
// trie root), all internal trie nodes are referenced jointly by database itself.
//
// It's only supported by hash-based database and will return an error for others.
